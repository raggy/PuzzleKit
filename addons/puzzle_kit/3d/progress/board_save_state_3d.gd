...
func apply_to_board(board: Board3D, group_filter: String = "") -> bool:
    ...
    if not _dereference_states_from_pieces(changed_pieces, board._pieces):
...
func _recreate_pieces(board: Board3D, new_pieces: Array[Piece3D]) -> bool:
    ...
    for state in created_pieces:
        ...
        if state.checkpoint_parent_piece_ref:
            if not state.checkpoint_parent_piece_ref.dereference_from(new_pieces) or not state.checkpoint_parent_piece_ref.dereference_from(board._pieces):
...
func _dereference_states_from_pieces(states: Array[PieceSaveState3D], pieces: Array[Piece3D]) -> bool:
    var success: bool = true

    for piece_save_state in states:
        success = success and piece_save_state.piece_ref and piece_save_state.piece_ref.dereference_from(pieces)
        if piece_save_state.parent_piece_ref:
            success = success and piece_save_state.parent_piece_ref.dereference_from(pieces)
        if piece_save_state.checkpoint_parent_piece_ref:
            success = success and piece_save_state.checkpoint_parent_piece_ref and piece_save_state.checkpoint_parent_piece_ref.dereference_from(pieces)
